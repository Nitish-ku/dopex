  ...
  return (
    <div className='min-h-screen flex flex-col items-center justify-start bg-[#0b0f14] p-6 text-white'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-gradient-to-br from-[#111827] to-[#0b1220] rounded-lg border border-gray-800'>
        ...
        <textarea onChange={(e)=> setInput(e.target.value)} value={input} rows={4} className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-700 bg-gray-900 text-gray-300' placeholder='Describe what you want to see in the image...' required/>

        <p className='mt-4 text-sm font-medium'>Style</p>

        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {imageStyle.map((item)=>(
            <span onClick={()=> setSelectedStyle(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedStyle === item ? 'bg-green-50 text-green-700' : 'text-gray-400 border-gray-700'}`} key={item}>{item}</span>
          ) )}
        </div>
      ...
      <div className='w-full max-w-lg p-4 bg-gradient-to-br from-[#111827] to-[#0b1220] rounded-lg flex flex-col border border-gray-800 min-h-96'>
