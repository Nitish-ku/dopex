  ...
  return (
    <div className='min-h-screen flex flex-col items-center justify-start bg-[#0b0f14] p-6 text-white'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-gradient-to-br from-[#111827] to-[#0b1220] rounded-lg border border-gray-800'>
        ...
        <input onChange={(e)=> setInput(e.target.value)} value={input} type="text" className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-700 bg-gray-900 text-gray-300' placeholder='The future of Artificial Intelligence is ....' required/>

        <p className='mt-4 text-sm font-medium'>Article Length</p>
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {articleLength.map((item, index)=>(
            <span onClick={()=> setSelectedLength(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLength.text === item.text ? 'bg-blue-50 text-blue-700' : 'text-gray-400 border-gray-700'}`} key={index}>{item.text}</span>
          ) )}
        </div>
      ...
      <div className='w-full max-w-lg p-4 bg-gradient-to-br from-[#111827] to-[#0b1220] rounded-lg flex flex-col border border-gray-800 min-h-96 max-h-[600px]'>
